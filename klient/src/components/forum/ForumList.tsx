    ...
    <div className="max-w-5xl mx-auto p-4">
      <div className="flex items-center justify-between mb-4">
        <div className="flex gap-2">
          ...
          <button onClick={load} className="px-4 rounded-xl bg-indigo-600 text-white">Filtruj</button>
        </div>
        <Link to="/forum/new" className="px-4 py-2 rounded-xl bg-indigo-700 text-white">+ Nowy wpis</Link>
      </div>

      {loading ? (
        <div className="text-slate-500">Ładowanie…</div>
      ) : items.length === 0 ? (
        <div className="text-slate-500">Brak wyników.</div>
      ) : (
